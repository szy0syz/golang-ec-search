...
BFF是be for fe的缩写，意思是服务于前端的后端，它是一个逻辑分层的一种思想，相当于增加了一层适配服务，专门用户后端服务的适配，给Web端、PC端、移动端等提供统一切友好的API，方便各端的接入。

#### 直接调用内部服务的问题

- 无线应用直接调用微服务会跟内部微服务重度耦合
- APP端实现一个功能可能需要调用多个内部服务
- APP端屏幕太小，需要对返回结果做处理，裁剪屌一些不需要的内容

#### 增加API Gateway

- 将BFF按团队或业务进行解耦拆分
- 认证、限流等功能全部放到API网关这层来实现
- 网关给还起到了负载均衡的作用，同时也解决了BFF层单点的问题

![WX20221023-161527@2x](https://user-images.githubusercontent.com/10555820/197382013-33294884-e6b3-4297-a862-bb43484abd0d.png)

#### BFF和网关的区别

- 网关负责负载均衡，鉴权验证，限流防爬等功能
- BFF对端进行适配裁剪聚合等功能
...
- RocketMQ的金融级稳定性和低延时适合于金融类业务
- Apache Pulsar 目前活跃度和成熟度不高，但值得关注
- 优先选择团队正在使用的，或者熟悉的，比如Kafka

> RocketMQ和Kafka的区别之一：RocketMQ是每个人单独做核酸，Kafka是混采(10混1)。
...
_,_,err := producer.SendMessage(&sarama.ProducerMessage{
  Topic: "test",
  Value: sarama.ByteEncoder("ticket1")
})
```
...
- `acks=0`
  - 代表只要消息一发出去，不等持久化就等于该消息已发送，就会继续发送下一条消息
  - kafka不管你服务端收到没
- `acks=1`
  - 只需要kafka中一个节点接受成功就认为发送成功
- `acks=-1/all`
  - 根据系统配置，有N个副本节点都接收成功了才算发送成功
  - 但它的延时也是最高的

注意事项

- 不是所有场景下我们都需要严格控制消息的丢失
  - 根据业务的需要来衡量
  - 我们需要做的是尽可能减少性能损失的前提下尽量避免消息丢失

如何尽可能避免消息的丢失？

> 如果业务对消息的容忍度很低，可是设置`acks=-1/all`

设置发送重试次数

```go
config.producer.Retry.Max = 5 // 默认是3
```
...
- 之多一次(At most once)
  - 多用于日志场景
- 至少一次(At least once)
  - 确保至少被传递一次，确保消息不丢失，但可能存在重复送达的情况
- 只有一次(Exactly once)
  - 消息在传递过程中只会送达一次，不允许消息丢失，也不允许重复投递，达到最高级别的可靠性保障，但时间成本最高
...
- 通过Token机制或者全局唯一ID的机制记录消息的消费状态来实现
  - 这种方案比较通用
  - 在发送消息时给每条消息生成一个全局唯一的ID，在消费时先检查这个ID是否已经消费，没有消费则执行操作后再更新状态
  - 但这样在分布式系统中会牺牲性能，都需要原子操作

#### 如何处理消费乱序的问题

设置分区策略为Hash

> 设置Hash就是为了让消息发送到相同的partition上，保障不乱序，并且可以自己手动指定

哪些情况会触发kafka消费者的rebalance呢？

> 如果触发，kafka的消费者会停止消费，直到rebalance结束，这显然会影响到消费的实时性，还会造成一段时间内的消息堆积。

- 消费组中的消费者加入或离开，这也是发生最多的情况
- partion数量的变化
- 对topic的订阅发生变化

如何避免消费者的rebalance？

> 当我们修改了kafka的相关配置时，特别是修改了topic的partion数量以后，必须重启生产者和消费者。

- 将消费者代码尽可能隔离出来，拆分成单独的微服务
  - 这样可以避免非消费部分的代码变更而导致发版发版需要重启
  - 需要确保消费者的代码逻辑的健壮性，必要的时候需要捕获抛出的panic，避免因为代码执行异常而导致的应用重启，从而触发消费者的rebalance
- 合理设置这个 `max.poll.interval.ms` 这个参数
  - 消费者长时间不提消息，这时kafka会判断它会死亡，从而被提出消费组，触发了消费者的rebalance
- 合理设置 `session.timeout.ms` 和 `heartbeat.interval.ms`
  - 消费者未能发送合理的心跳而被提出

> 整球不成，代码太难了，还是先写demo去。
